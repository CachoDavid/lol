...
export function championsToChampionSummaries(
  arr: ChampionListItem[]
): ChampionSummary[] {
  return arr.map(({ id, name, title, info, tags, image }) => ({
    id,
    name,
    title,
    info: {
    ...
    image: `https://ddragon.leagueoflegends.com/cdn/13.13.1/img/champion/${image.full}`,
    tags
  }))
}
...
export function championItemToChampion(api: ChampionItem): Champion {
  const champion: Champion = {
    ...
    spells: api.spells,
    skins: api.skins,
    image: `https://ddragon.leagueoflegends.com/cdn/13.13.1/img/champion/${api.image.full}`,
    tags: api.tags
  }
  return champion
}
